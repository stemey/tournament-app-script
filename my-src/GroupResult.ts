...
export class GroupResult {
  private resultMap: { [player: string]: Result[] };
  private matches: { [key: string]: MatchResult } = {};
  ...
  private calculateStats(player): Stats {
    ...
    if (!results) {
      return { setpoints: [0, 0], sets: [0, 0], matches: [0, 0], ranking: -1 };
    }
    return results.reduce(
      ...
      { setpoints: [0, 0], sets: [0, 0], matches: [0, 0], ranking: -1 } as Stats
    );
  }
}
