...
function createMatchForm() {
  ...
  form.addListItem()
    .setTitle('Spieler/Team 1')
    .setChoiceValues(players);
  form.addListItem()
    .setTitle('Spieler/Team 2')
    .setChoiceValues(players);

  var textValidation = FormApp.createTextValidation()
    .requireTextMatchesPattern("[0-7]:[0-7][ ,]+[0-7]:[0-7]([ ,]+[0-9]+:[0-9]+)?")
...
function createRegistrationForm() {
  ...
  const playerGroups = getPlayerGroups()
  let players = [];
  playerGroups.forEach(g => players = players.concat(g.players));
