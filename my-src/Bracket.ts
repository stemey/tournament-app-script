...
function renderBracket() {
  ...
  const numPlayers =
    Browser.inputBox("Wieviele Spieler nehmen an der KO Runde teile?") || 16;
 ...
 */
function setBracketItem_(cell, player) {
  cell.setBackgroundColor("yellow");
  const opponent = cell.offset(1, 0);

  opponent.setBackgroundColor("yellow");

  cell.offset(0, 1, 2, 3).setBackgroundColor("lightgrey");

  cell.getSheet().setColumnWidth(cell.getColumnIndex() + 0, PLAYER_WIDTH);
  cell.getSheet().setColumnWidth(cell.getColumnIndex() + 1, CONNECTOR_WIDTH);
  cell.getSheet().setColumnWidth(cell.getColumnIndex() + 2, CONNECTOR_WIDTH);
  cell.getSheet().setColumnWidth(cell.getColumnIndex() + 3, CONNECTOR_WIDTH);
}
